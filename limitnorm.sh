...
MANUAL="limitnorm.sh script manual:
    ...
    USAGE:
        ...
        limitnorm.sh [options...] [folder or files...]
    ...
    EXAMPLES:
        limitnorm.sh . --- to convert whole current folder to a subfolder named \"Normalized\" (default)
        limitnorm.sh 0.wav 1.wav --- to convert individual files to subfolder called \"Normalized\" within each file's folder
        limitnorm.sh --output-folder Norm --copy WAV --- to normalize all files inside WAV/ folder to a subfolder named \"Norm\" and copy any untouched files
        limitnorm.sh -o ~/output -c BiBs --- to convert all files inside BiBs/ to ~/output and copy any untouched files"
...
for file in $files; do
  ...
  if [ $(echo "$integrated > -15.2" | bc -l) -eq 1 ] || [ $(echo  "$integrated < -16.8" | bc -l) -eq 1 ] || [ $(echo  "$truepeak > -2.6" | bc -l) -eq 1 ]; then 
    ...
    if [ $(echo "$plr > 13.0" | bc -l) -eq 1 ]; then
      # "alimiter=level=false:limit=-4.0dB"
      COMMAND+="\"alimiter=level=false:level_in=${level}dB:limit=-3.0dB\""
    else
      COMMAND+="\"volume=${level}dB\""
    fi
